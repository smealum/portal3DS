...
typedef struct pileCell_s
{
	vect3Di_s data;
	struct pileCell_s* next;
}pileCell_s;
...
void initPile(pile_s* p)
{
	p->first=NULL;
	p->num=0;
}

void pushPile(vect3Di_s v, pile_s* p)
{
	pileCell_s* pc=(pileCell_s*)malloc(sizeof(pileCell_s));
	pc->next=p->first;
	pc->data=v;
	p->first=pc;
	p->num++;
...
vect3Di_s popPile(pile_s* p)
{
	p->num--;
	if(p->first)
	{
		pileCell_s* pc=p->first;
		vect3Di_s v=pc->data;
		p->first=pc->next;
		free(pc);
		return v;
...
int area(vect3Di_s origin, vect3Di_s corner)
{
...
{
	int x;
	vect3Di_s originb=vect2(0,0),cornerb=vect2(-1,-1);
	pile_s p;
	initPile(&p);
	int* c=initCache(h);
	for(x=w-1;x>=0;x--)
...
listCell2D_s* createListCell()
{
	listCell2D_s* lc=(listCell2D_s*)malloc(sizeof(listCell2D_s));
	lc->next=NULL;
	return lc;
}
...
void insertRectangle2DList(rectangle2DList_s* l, rectangle2D_s rec)
{
	...
	listCell2D_s* n=createListCell();
	n->data=rec;
	listCell2D_s** plcn=&l->first;
	listCell2D_s* lc=l->first;
	l->num++;
	l->surface+=a;
	if(n->data.real)n->data.real->lmSize=vect3Di(n->data.size.x,n->data.size.y,0);
	while(lc)
	{
...
void initTreeNode(treeNode_s* tn)
{
...
treeNode_s* createNode()
{
	treeNode_s* tn=(treeNode_s*)malloc(sizeof(treeNode_s));
	initTreeNode(tn);
	return tn;
}

void initTree(tree_s* t, short w, short h)
{
	t->root=createNode();
	t->width=w;
	t->height=h;
...
void freeTreeBranch(treeNode_s* tn)
{
	if(tn)
	{
...
bool insertRectangle(treeNode_s* tn, rectangle2D_s* rec, short w, short h, short x, short y, int a)
{
	bool r;
...
bool insertRectangles(tree_s* t, rectangle2DList_s* l)
{
	listCell2D_s*lc=l->first;
	bool rr=true;
	while(lc)
	{
		...
		if(r)
		{
			...
			printf("pos %p %d %d %d %d (%d)\n",lc->data.real,lc->data.real->lmPos.x,lc->data.real->lmPos.y,lc->data.size.x,lc->data.size.y,lc->data.rot);
		}else rr=false;
		lc=lc->next;
	}
	return rr;
...
void unrotateRectangles(rectangle2DList_s* l)
{
	if(!l)return;
	listCell2D_s*lc=l->first;
	while(lc)
	{
		rectangle2D_s* rec=&lc->data;
		if(rec->rot)
		{
			int z=rec->size.x;
			rec->size.x=rec->size.y;
...
// int32 pointRectangleDistance(vect3D o, rectangle_s* rec)
// {
// 	if(!rec->size.x)
// 	{
